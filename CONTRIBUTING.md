...
Pull Request を出す前に、少なくとも **`bun run lint`** と **`bun run check`** と **`bun run test`** と **`bun run build`** が通る状態にしてください。GitHub Actions では `.github/workflows/ci-*.yml` が同じイベントで並列に走り、チェックがジョブ単位に分かれます。
...
**リポジトリのステージングブランチ**（`develop`）向けが標準です。
